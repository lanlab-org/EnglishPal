...
def mark_word():
    if request.method == 'POST':
        d = Sql.load_freq_history(path_prefix + 'static/frequency/frequency.p')
...
def mainpage():
    if request.method == 'POST':  # when we submit a form
        ...
        lst = f.get_freq()
        # save history
        d = Method.load_freq_history(path_prefix + 'static/frequency/frequency.p')
        lst_history = pickle_idea.dict2lst(d)
        d = pickle_idea.merge_frequency(lst, lst_history)
        pickle_idea.save_frequency_to_pickle(d, path_prefix + 'static/frequency/frequency.p')
        x = 1
        if len(d) > 0:
            for i in sort_in_descending_order(pickle_idea.dict2lst(d)):
                ...
                num[x] = i[1]
                x = x + 1
        return render_template('home.html', youdao=youdao, word=word, num=num, x=x)
        ...
        random_ads = Method.get_random_ads()
        d = Method.load_freq_history(path_prefix + 'static/frequency/frequency.p')
        x = 1
        if len(d) > 0:
            for i in sort_in_descending_order(pickle_idea.dict2lst(d)):
                ...
                num[x] = i[1]
                x = x + 1
        return render_template('homepage.html', user=user, random_ads=random_ads, youdao=youdao, word=word, num=num,
                               x=x)
...
def user_mark_word(username):
    ...
    if request.method == 'POST':
        ...
        lst = []
        for word in request.form.getlist('marked'):
            lst.append((word, [Method.get_time()]))
...
def userpage(username):
    ...
    if datetime.now().strftime('%Y%m%d') > user_expiry_date:
        username = ''
        username = session['username']
        return render_template('out_of_date.html', username=username)
    ...
    if request.method == 'POST':  # when we submit a form
        ...
        for x in lst:
            ...
            count, youdao_link(x[0]), Method.appears_in_test(x[0], words_tests_dict), x[0], x[1], x[0])
            count += 1
        page += '</form>\n'
        return page
        ...
        page += '<div id="text-content">%s</div>' % (Method.get_today_article(user_freq_record, session['articleID']))
...
def signup():
    if request.method == 'GET':
        ...
        available = Sql.Sql.check_username_availability(username)
        if not available:
